...
cli cloud functions list

[查看云函数信息](#查看云函数信息)

cli cloud functions info
...
cli cloud functions inc-deploy

[下载云函数](#下载云函数)

cli cloud functions download
...
小程序 AppID 或第三方平台 AppID。如果有提供 --project，该选项将忽略

\--ext-appid

第三方平台开发时被开发 AppID。如果有提供 --project，该选项将忽略
...
可选。工具 HTTP 服务端口。如果工具没有启动，则会启动并在给定端口号启动服务。如果工具已启动并且端口不同，需要先退出工具再重新执行。

开启调试模式

\--debug

可选。开启调试模式，用于输出额外信息以协助定位问题
...
打开自动化测试窗口，同时可以开始回放之前录制好的测试用例。 录制回放可[点此](auto/record)查看。 最低可用工具版本：`1.05.2111232`
