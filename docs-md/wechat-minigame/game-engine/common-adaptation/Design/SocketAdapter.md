# [#](#TCP-UDP-Socket-适配) TCP/UDP Socket 适配
...
## [#](#POSIX-接口支持情况) POSIX 接口支持情况

### [#](#_1-原生支持的接口（无需修改）) 1. 原生支持的接口（无需修改）

以下接口与标准 POSIX 接口完全兼容，可直接使用：

#### [#](#_1-1-Socket-创建与关闭) 1.1 Socket 创建与关闭
...
> ⚠ **注意**：小游戏环境基于安全考虑，在非调试模式时不支持直接 IP 地址连接，需要采用域名连接。参考 [微信小游戏网络使用说明](../../../base-ability/network) 前往 MP 配置相关域名，然后采用域名接口建立连接。
> 
> 域名连接接口：
> 
> ```c
...
### [#](#_2-需要修改的接口) 2. 需要修改的接口

#### [#](#_2-1-select-需要使用-WXSocket-命名空间) 2.1 select() - 需要使用 WXSocket 命名空间
...
#### [#](#_2-2-ioctl-仅支持-FIONREAD) 2.2 ioctl() - 仅支持 FIONREAD
...
#### [#](#_2-3-connect-支持域名连接（扩展接口）) 2.3 connect() - 支持域名连接（扩展接口）
...
#### [#](#_2-4-close-关闭-socket) 2.4 close() - 关闭 socket
...
### [#](#_4-Emscripten-编译选项配置) 4. Emscripten 编译选项配置

#### [#](#_4-1-必需的编译选项) 4.1 必需的编译选项
...
不允许内存动态增长（建议通过 `INITIAL_MEMORY` 配置合理值，避免内存动态增长）
...
### [#](#_5-完整-Makefile-示例) 5. 完整 Makefile 示例
...
> ⚠ 该库只能在微信小游戏环境下使用，浏览器环境不支持。请将上述 Emscripten 构建产物利用微信小游戏转换工具进行转换，具体请参考 [转换工具](TransformTool)。

* * *

## [#](#使用示例) 使用示例

### [#](#示例-1：TCP-客户端连接) 示例 1：TCP 客户端连接
...
### [#](#示例-4：UDP-连接模式（connect-send-recv）) 示例 4：UDP 连接模式（connect + send/recv）
...
#### [#](#JavaScript-库未加载) JavaScript 库未加载
...
-   了解 [WebSocket 适配](WebSocketAdapter)
-   了解 [HTTP 客户端适配](HttpAdapter)
-   了解 [微信开放接口](WXOpenAPI)
