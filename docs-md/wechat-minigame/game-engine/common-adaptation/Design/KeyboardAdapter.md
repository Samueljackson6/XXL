...
#### [#](#KeyboardConfirmType-确认按钮类型) KeyboardConfirmType - 确认按钮类型
...
#### [#](#KeyboardType-键盘类型) KeyboardType - 键盘类型
...
#### [#](#KeyboardEventType-键盘事件类型) KeyboardEventType - 键盘事件类型
...
### [#](#配置接口-KeyboardConfigInterface) 配置接口 (KeyboardConfigInterface)
...
`SetInputCallback(callback, user_data)`

设置输入变化回调

`SetConfirmCallback(callback, user_data)`
