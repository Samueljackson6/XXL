...
支持 `fopen`/`fread`/`fwrite`/`fseek`/`ftell`/`fclose` 等标准接口

目录操作

支持 `mkdir`/`rmdir`/`access` 等标准接口

路径自动映射

以 `/CustomWritablePath` 开头的路径自动映射到微信文件系统

扩展接口

递归删除目录、TAR 包加载、ZIP 解压、文件同步等
...
### [#](#TAR-包加载-vs-ZIP-解压) TAR 包加载 vs ZIP 解压
...
整个 TAR 包一次性读入内存，建立文件索引后直接从内存读取

异步解压到磁盘，后续通过文件系统 API 逐个读取

**并发读取**

✅ 纯内存操作，多个文件可同时读取，无 I/O 竞争

⚠ 每次读取都是一次磁盘 I/O 调用，并发时存在 I/O 竞争

**读取性能**

零拷贝内存视图，读取延迟极低（微秒级）

首次需调用微信文件系统 API 读取磁盘，延迟较高（毫秒级）

**磁盘占用**

不解压到磁盘，不额外占用用户存储空间

解压后文件占用磁盘空间（原始文件 + 解压文件）
...
高频并发读取的只读资源（Lua 脚本、配置文件、美术资源等）

需要修改的文件，或包体较大不适合常驻内存的场景
...
### [#](#syncCreateFile-接口说明) syncCreateFile 接口说明
...
文件的虚拟路径，如 `/CustomWritablePath/config/settings.json`
...
-   **零散小文件合并**：大量零散小文件（如配置文件、脚本等）建议合并存储，可采用 TAR 包（参考上文 [TAR 包加载](#TAR-包加载)）或自定义打包格式，减少文件系统调用开销
