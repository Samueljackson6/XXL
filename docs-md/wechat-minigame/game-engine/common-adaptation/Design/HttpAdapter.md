...
GET, POST, PUT, DELETE, HEAD, OPTIONS 等方法
...
## [#](#HttpClientInterface-详细配置) HttpClientInterface 详细配置
...
`SetSuccessCallback(callback, user_data)`

设置成功回调

`SetFailCallback(callback, user_data)`
...
`SetBody(type, data, size)`

设置请求体（文本或二进制）

`SetResponseType(type)`

设置响应类型（kResponseTypeString / kResponseTypeBinary）

`SetCompleteCallback(callback, user_data)`
...
`SetEnableQuic(enable)`

启用 QUIC/H3 协议

`SetEnableHttpDNS(enable)`

启用 HttpDNS

`SetHttpDNSServiceId(serviceId)`
...
`SetEnableChunked(enable)`

启用分块传输

`SetUseHighPerformanceMode(enable)`

启用高性能模式

`SetHeaderReceivedCallback(callback, user_data)`

设置响应头接收回调

`SetChunkReceivedCallback(callback, user_data)`

设置分块数据接收回调

## [#](#HttpDownloadClientInterface-配置) HttpDownloadClientInterface 配置
...
本地保存路径（必须以 `/CustomWritablePath` 开头）
...
## [#](#HttpUploadClientInterface-配置) HttpUploadClientInterface 配置
...
-   了解 [文件系统适配](FileSystemAdapter)
-   了解 [键盘适配](KeyboardAdapter)
-   了解 [TCP/UDP Socket 适配](SocketAdapter)
-   了解 [WebSocket 适配](WebSocketAdapter)
-   了解 [微信开放接口](WXOpenAPI)
-   了解 [UE HTTP 集成](UEHttpIntegration)
