...
### [#](#如何通过错误堆栈查找源代码问题) 如何通过错误堆栈查找源代码问题

#### [#](#使用Profiling-funcs) 使用Profiling-funcs

转换面板勾选profiling-funcs, 导出的代码包中将包含可读函数名。 正式上线版本请务必关闭profiling-funcs，或使用代码分包(可自动剔除函数名）。

#### [#](#使用symbols文件可读函数名) 使用symbols文件可读函数名
...
这里有个[小工具](Symbol)可以帮助替换日志(在WASM代码分包情况下，需要自行稍做修改)

## [#](#影响异常错误的导出选项) 影响异常错误的导出选项
