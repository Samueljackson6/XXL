# [#](#微信小游戏压缩纹理工具2-0-Beta) 微信小游戏压缩纹理工具2.0(Beta)
...
Unity 2019支持部分；Unity 2020支持部分格式；Unity 2021支持

2019、2020、2021均有版本支持

格式支持

Unity 2019支持DXT等；Unity 2020支持DXT、ETC，但不支持ASTC；Unity 2021支持多数
...
## [#](#WebGL2-0-支持说明（Beta）) WebGL2.0 支持说明（Beta）

​ 微信纹理压缩目前已支持WebGL2.0模式下的Gamma与Linear颜色空间渲染，但仅支持2019、2020、2021 3个年限中部分版本，其中2019必须使用Unity与微信合作的[Instant Game版本](UnityVersion#基于2019429)，其余年限跟随微信纹理压缩支持版本进行选择。该能力仍在兼容一些特殊的应用场景，开发者使用后需要进行详细的真机测试。
...
​ 微信纹理压缩升级了全新的内核程序，尽管发布前进行了多数Unity年限版本与常见机型的测试，目前仍处于初期测试使用，若使用期间存在异常影响正常的游戏发布可前往issues提出问题，并使用[上一代稳定版微信纹理压缩包（旧）](https://git.weixin.qq.com/wechat-minigame/minigame-tuanjie-demo/blob/master/tools/minigame.texturecompressed.backup.unitypackage)导入工程后采用旧版本完成游戏发布。旧版本使用指引请参阅：[微信压缩纹理使用说明（旧）](CompressedTextureOld)
...
### [#](#_5-分离纹理后各自单独下载是否会增加下载开销？) 5.分离纹理后各自单独下载是否会增加下载开销？
...
### [#](#_6-MacOS系统执行报错-“-System-Drawing-GDIPlus-threw-an-exception”) 6.MacOS系统执行报错 “'System.Drawing.GDIPlus' threw an exception”
...
### [#](#WXAssetsTextTools-CompressText) WXAssetsTextTools.CompressText( ... )
...
执行结束的回调函数，形参1(bool) 返回执行是否成功，形参2(string)返回执行错误时的错误提示信息。

bundleDir

string

可缺省，若存在独立的AB包目录，则此处填写目录的绝对路径。

outBundleDir

string

可缺省，若填写bundleDir则不可省略，独立AB包处理后的输出目录。

debug

bool

可缺省，默认全量生成(false)，true时仅生成ASTC格式纹理。

force

bool

可缺省，默认遵循增量逻辑(false)，true时将强制生成（已生成资源仍然生成并覆盖）。
...
### [#](#WXAssetsTextTools-GetAssetBundles) WXAssetsTextTools.GetAssetBundles( ... )
...
扫描结束的回调函数，形参(string\[\]) 返回扫描到的ab包资源绝对路径。
