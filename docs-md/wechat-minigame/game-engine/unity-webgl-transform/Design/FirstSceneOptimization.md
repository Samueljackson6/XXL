...
在[提升Unity WebGL游戏启动速度](StartupOptimization)章节我们提到过：
...
## [#](#Unity-CallMain) Unity CallMain

参考文档[小游戏启动流程](Startup)中提到的，当游戏代码包下载编译完成和首包资源下载完成后会运行游戏逻辑，最开始执行的逻辑是CallMain主函数。 在WebGL模式下，CallMain的主要是initWebGLPlayer调用，主要分成两部分调用：
...
## [#](#CallMain-Profile分析) CallMain Profile分析
