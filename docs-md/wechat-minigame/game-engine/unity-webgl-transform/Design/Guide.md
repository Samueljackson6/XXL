# [#](#Unity-游戏接入微信小游戏指南) Unity 游戏接入微信小游戏指南
...
> 相关手册：[兼容性评估](Evaluation) 、[推荐引擎版本](UnityVersion)、[更多小游戏成功转换案例](ShowCase) 、[技术常见问题QA](DevelopmentQAList)
...
### [#](#快速开始——转换工具导出微信小游戏) 快速开始——转换工具导出微信小游戏

> 相关下载： [微信 Unity 插件下载](https://game.weixin.qq.com/cgi-bin/gamewxagwasmsplitwap/getunityplugininfo?download=1)
> 
> 相关手册：[快速开始：转换工具导出微信小游戏](Transform)

​ 阅读 **快速开始：转换工具导出微信小游戏** 快速熟悉工具的使用并完成一次简单的转化工作！

### [#](#资源按需加载) 资源按需加载

> 相关手册：[资源按需加载概述](ResourcesLoading) 、[AA(Addressable) 进行资源按需加载](UsingAddressable) 、 [AB(AssetBundle)进行资源按需加载](UsingAssetBundle) 、[Instant Game 实践指南](InstantGameGuide)
...
> 相关手册：[后端服务指引](BackendServiceStartup) 、[网络通信适配](UsingNetworking)
...
> 相关手册：[资源部署与缓存](DataCDN) 、[资源缓存](FileCache) 、[iOS高性能模式](iOSOptimization)
...
> 相关手册：[开发错误调试与排查](DebugAndException)

小游戏存在开发者工具、真机(Android/iOS/PC Windows)，当游戏出现异常时可通过日志进行错误堆栈分析。

## [#](#【阶段三】微信平台能力接入) 【阶段三】微信平台能力接入

### [#](#接入微信API) 接入微信API

> 相关手册：[WX SDK 平台能力适配](WX_SDK) 、[屏幕适配](fixScreen) 、 [输入法适配](InputAdaptation) 、 [排行榜与微信关系数据](OpenData)

​ Unity 游戏接入微信小游戏平台将获得微信提供的 API 以及开放能力，开发者根据需要进行按需接入。微信 API 支持的能力包括登录、设备（存储、震动）、开放数据、广告等等。

### [#](#启动留存数据上报统计) 启动留存数据上报统计

> 相关手册：[启动留存数据上报统计](ReportStartupStat)

​ 在小游戏中玩家对启动时长与体验十分敏感（尤其从“广告”等买量场景进入的玩家），因此需要在必要的位置进行相关的数据上报，[Unity Loader 插件](UsingLoader)为游戏提供了一些基础上报数据，但游戏内部关键帧位置仍需要开发者自行上报，可参阅相关手册完成上报配置。

## [#](#【阶段四】体验调优) 【阶段四】体验调优

​ 截止本阶段，开发者的 Unity 游戏将在微信小游戏平台成功启动运行，但为了能够达到更佳的游戏体验开发者应继续进行对游戏工程的调优工作，本节将介绍目前微信小游戏平台为开发者提供的调优能力完成上线前的最后优化工作。

### [#](#首场景启动优化——首帧逻辑优化) 首场景启动优化——首帧逻辑优化

> 相关手册：[提升Unity WebGL游戏启动速度](StartupOptimization) 、[启动流程与时时序](Startup) 、 [首场景启动优化](FirstSceneOptimization)
...
> 相关手册：[使用预下载功能](UsingPreload)

​ 在 UnityLoader 加载过程中是存在CPU处理密集而 **网络空闲** 的情况，因此可以充分利用该时机进行一些必要的资源下载可有助于提升游戏场景等资源的载入速度。

### [#](#运行性能及调优) 运行性能及调优

> 相关手册：[优化Unity WebGL的运行性能](OptimizationPerformence)、[使用 Android CPU Profiler 性能调优](AndroidProfile)、[使用 Unity Profiler 性能调优](UnityProfiler)、[优化 Unity WebGL 内存](OptimizationMemory)

​ 游戏转化完成后仍需要对各项的性能指标进行量化测试。

### [#](#压缩纹理资源) 压缩纹理资源

> 相关手册： [压缩纹理优化](CompressedTexture)
...
## [#](#【阶段五】发布上线与现网监控) 【阶段五】发布上线与现网监控

​ 审核发布可参考 MP 文档 [小游戏接入指南](Guide) 自助完成，游戏相关运行数据请前往 [微信公众平台](https://mp.weixin.qq.com/)。

​ [现网错误日志上报与排查](IssueForProduction)
