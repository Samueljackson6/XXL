# [#](#微信小游戏引擎适配概述) 微信小游戏引擎适配概述

## [#](#一、平台与引擎的关系) 一、平台与引擎的关系
...
## [#](#二、为什么需要引擎适配) 二、为什么需要引擎适配

不同游戏引擎有各自的编译产物形式和运行时依赖：

-   **HTML5 引擎**（如 Cocos Creator、Laya、Egret）：编译产物为 JavaScript 代码，运行时通常依赖浏览器的 BOM 和 DOM API。但小游戏环境并非浏览器，没有 `document`、`window` 等全局对象，因此需要通过[适配层（Adapter）](../runtime/adapter)模拟这些 API，或由引擎官方完成小游戏平台的原生适配。
...
### [#](#_3-1-JavaScript-引擎适配) 3.1 JavaScript 引擎适配
...
### [#](#_3-2-WebAssembly-引擎适配) 3.2 WebAssembly 引擎适配
...
[Cocos/Laya/Egret 引擎适配](cocos-laya-egret)
...
[Unity/团结引擎适配](unity-webgl-transform)
