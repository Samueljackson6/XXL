...
如果你使用的是[Unity](../game-engine/unity-webgl-transform)、[Cocos 或者 Laya](../game-engine/cocos-laya-egret)等游戏引擎进行开发，在游戏引擎侧就能导出小游戏代码包，通过面板中的【导入】，选择对应的文件夹目录打开即可
...
**注意：小游戏创建项目默认创建的飞机游戏示例，本质上是一个 Canvas2D 游戏，本文只提供一个游戏开发的思路讲解，并不推荐在线上环境中使用该示例，我们更推荐使用[Unity](../game-engine/unity-webgl-transform)、[Cocos 或者 Laya](../game-engine/cocos-laya-egret)等游戏引擎进行小游戏开发**
...
-   注意：如果你想以 Web 开发的风格写小游戏代码，可以通过引入[weapp-adapter](../runtime/adapter)来实现。
...
如果你有引入 [weapp-adapter](../runtime/adapter)，在 [weapp-adapter](../runtime/adapter) 中默认会创建一个 Canvas 做为主屏 Canvas
...
在本示例中，当敌机和子弹发生碰撞时，会播放一个爆炸动画，并同时[播放音效](../base-ability/audio)和[震动](https://developers.weixin.qq.com/minigame/dev/api/device/vibrate/wx.vibrateShort.html)，具体代码可以查看`js/npc/enemy.js`中的`destroy`部分
