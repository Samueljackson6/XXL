...
6.  回包给微信服务器，首先需确定回包包体的明文内容，具体取决于特定接口文档要求，如无特定要求，回复空串或者success（无需加密）即可，其他回包内容需加密处理。这里假设回包包体的明文内容为"{"demo\_resp":"good luck"}"，数据格式为JSON，下面介绍如何对回包进行加密：
...
-   “调试工具”允许开发者填写[AccessToken](https://developers.weixin.qq.com/minigame/dev/api-backend/access-token/api_getaccesstoken)、Body后，微信服务器会拉取你在小程序后台配置的消息推送配置，实际推送一条debug\_demo事件供开发者调试。![](https://res.wx.qq.com/op_res/CyXI7cGX7jxeP-vdY2Hkm-hEi9O6s8jFZI7ewGAoi8bUp6X86EpNxCM2wn_rnHrhQ7ByOfhYVmrN-TLJVR69HQ
)
