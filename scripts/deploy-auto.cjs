#!/usr/bin/env node
/**
 * WeChat MiniGame Auto-Deploy Pipeline v4.0
 *
 * Fully automated deployment - no manual steps required.
 * Uses miniprogram-automator's launcher.launch() with cliPath for
 * proper IDE launch + wsEndpoint connection (handles everything internally).
 * 1. Verify and patch minigame files (.framework/ sync, game-init.js)
 * 2. Launch DevTools + connect via launcher.launch({ cliPath })
 * 3. Trigger compilation via Windows UI Automation (Ctrl+B)
 * 4. Monitor console for errors with Promise-based timeout
 * 5. Report results
 */
...
const DEVTOOLS_NODE = resolve(DEVTOOLS_DIR, 'node.exe');
const DEVTOOLS_CLI_JS = resolve(DEVTOOLS_DIR, 'cli.js');
...
function verifyAndPatchFiles() {
  logStep(1, 5, 'Verifying and patching minigame files...');
  ...
  // Patch: Ensure .framework/ mirrors framework/
  ...
  // Patch: Ensure game-init.js exists and is imported
...
// ============================================================================
// Step 2: Launch DevTools + Connect (hybrid approach)
// ============================================================================
...
// launcher.launch() internally does spawn(cliPath, args) without shell:true,
// which CANNOT execute .bat files on Windows. It also rejects .exe paths.
// The WORKING approach (proven by v3.1 + diagnose scripts):
//   shell:true + cli.bat → IDE starts on port 5000, wsEndpoint on port 9996
// Then connect via launcher.connect() with wsEndpoint on port 9996.

const DEVTOOLS_CLI_BAT = resolve(DEVTOOLS_DIR, 'cli.bat');

async function launchAndConnect() {
  logStep(2, 5, 'Launching DevTools and connecting...');
  killDevTools();
  log('  Spawning IDE with shell:true + cli.bat...');

  // Kill again after wait to ensure clean state
  execQuiet('taskkill //F //IM wechatdevtools.exe 2>nul');
  sleep(3000);
  ...
  for (let i = 0; i < 30; i++) {
    ...
    if (r.ok) {
      log(`  IDE HTTP ready (${((Date.now()-httpStart)/1000).toFixed(1)}s)`);
      break;
    }
    log(`  [${((i+1)*3)}s] waiting...`);
  }
  ...
  // Connect via launcher.connect() with wsEndpoint on port 9996
  // (wsEndpoint is on a DIFFERENT port from HTTP — discovered via diagnose-ws2)
  log('  Connecting via launcher.connect() ws://127.0.0.1:9996...');
  const { launcher } = require('miniprogram-automator');

  let mp;
  // Approach 1: Try wsEndpoint on port 9996 (discovered via diagnose-ws2)
  try {
    ...
    // Approach 2: Scan all ports with raw ws
    log('  Scanning all ports for ws endpoint...');
    const wsLib = require('ws');
    for (let scanPort = 5000; scanPort <= 65535; scanPort++) {
      if (scanPort === 53 || scanPort === 135 || scanPort === 139 || scanPort === 445) continue;
      const connected = await new Promise(resolve => {
        let done = false;
        const timer = setTimeout(() => { if (!done) { done = true; resolve(false); } }, 300);
        try {
          const sock = new wsLib('ws://127.0.0.1:' + scanPort + '/');
          sock.on('open', () => { if (!done) { done = true; clearTimeout(timer); sock.close(); resolve(true); } });
          sock.on('error', () => { if (!done) { done = true; clearTimeout(timer); resolve(false); } });
        } catch { if (!done) { done = true; clearTimeout(timer); resolve(false); } }
      });
      if (connected) {
        log(`  ws on port ${scanPort} — trying launcher.connect...`);
        try {
          mp = await launcher.connect({ wsEndpoint: `ws://127.0.0.1:${scanPort}` });
          log(`  Connected via ws://127.0.0.1:${scanPort}`);
          break;
        } catch {}
      }
    }
  ...
  if (!mp) {
    logErr('Cannot connect to DevTools by any method.');
    return null;
  }
...
async function triggerCompilation(mp) {
  logStep(3, 5, 'Triggering compilation...');
...
async function monitorConsole(mp) {
  logStep(4, 5, 'Monitoring runtime console...');
  ...
  mp.on('exception', (e) => {
    try {
      ...
      errors.push(msg);
      if (errors.length <= 50) {
        console.log(`  [EXCEPTION] ${msg.slice(0, 200)}`);
      }
    } catch {}
  });
  ...
  // Promise-based timeout with early exit on success
  await new Promise((resolve) => {
    ...
    const checkInterval = setInterval(() => {
      if (!monitorDone) {
        const hasInit = consoleEntries.some(e =>
          /game.*init|unity|loaded|ready|MiniGame|compilation|compile/i.test(e)
        );
...
function report(result) {
  ...
  console.log('  ├─────────────────────────────────────────────┤');
  console.log(`  │  Status:  ${success ? 'SUCCESS ✓' : 'ERRORS ✗'}                  │`);
  console.log(`  │  Time:    ${elapsed.padStart(6)}s                         │`);
  console.log(`  │  AppID:   ${APPID} │`);
  if (result.errors !== undefined) {
    console.log(`  │  Errors:  ${result.errors.toString().padStart(6)} │`);
  }
  console.log('  └─────────────────────────────────────────────┘');
  console.log('');
}
...
async function main() {
  ...
  console.log('║  Auto-Deploy Pipeline v4.0                ║');
  console.log('╚════════════════════════════════════════════╝');
  console.log('');
