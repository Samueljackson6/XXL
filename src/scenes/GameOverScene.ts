...
export default class GameOverScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.text(cx, 230, `存活波次: ${survived} / 30`, {
      fontSize: '20px',
      color: '#ffffff',
    }).setOrigin(0.5);

    this.add.text(cx, 270, `击杀敌人: ${killed}`, {
      fontSize: '20px',
      color: '#ffffff',
    }).setOrigin(0.5);

    const retryBtn = this.add.text(cx, 380, '[ 重新开始 ]', {
      fontSize: '22px',
      fontFamily: 'Arial, sans-serif',
      color: '#4fc3f7',
    ...
    const menuBtn = this.add.text(cx, 440, '[ 返回主菜单 ]', {
      fontSize: '18px',
      fontFamily: 'Arial, sans-serif',
      color: '#888888',
    }).setOrigin(0.5).setInteractive({ useHandCursor: true });
